...
class LocalAdventureManager @Inject constructor(
    ...
    suspend fun loadHomeUiState(games: List<GameConfigItem>): AdventureHomeUiState = withContext(Dispatchers.IO) {
        ...
        if (definitions.isEmpty()) {
            return@withContext AdventureHomeUiState(
                ...
                currentChapterTitle = currentMeta.chapterTitle,
                totalExp = profile.totalExp,
                totalCoins = profile.totalCoins,
                nextLevelPreview = currentMeta.nextLevelPreview,
        ...
        val requiredTaskCount = loadCachedChapterSpec(profile.currentChapterId)?.requiredTaskCount
            ?: loadAdventureConfig().chapters.firstOrNull { it.chapterId == profile.currentChapterId }?.requiredTaskCount
            ?: 7
        ...
        AdventureHomeUiState(
            ...
            upgradeReady = completedCount >= requiredTaskCount,
    ...
    suspend fun claimTaskReward(taskId: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            if (progress.status != AdventureTaskStatus.ACHIEVED.name) return@withContext false
            ...
            val def = dao.getTaskDefinitionsByChapter(progress.chapterId)
                .firstOrNull { it.taskId == taskId }
            val expGain = def?.rewardExp?.toLong() ?: 40L
            val coinsGain = def?.rewardCoins?.toLong() ?: 12L
     ...
     */
    suspend fun tryLevelUp(): Boolean = withContext(Dispatchers.IO) {
        try {
            val profile = dao.getProfile() ?: return@withContext false
            val levelMetas = buildLevelMetas()
            ...
            if (completedCount < requiredCount) return@withContext false

            val nextMeta = levelMetas.firstOrNull { it.level == currentMeta.level + 1 }
                ?: return@withContext false // Already max level
    ...
    private suspend fun ensureChapterSeed(chapterId: String, games: List<GameConfigItem>) {
        val config = loadAdventureConfig()
        val chapterSpec = loadCachedChapterSpec(chapterId)
        ...
        // Compute the global game offset: chapter_01→0, chapter_02→10, chapter_03→20, …
        val chapterIndex = config.chapters.indexOfFirst { it.chapterId == chapterId }.coerceAtLeast(0)
        val gameOffset = chapterIndex * 10

        val definitions = chapterSpec.tasks.mapIndexed { slotIdx, taskSpec ->
            val realGame = games.getOrNull(gameOffset + slotIdx)
            ...
            LocalAdventureTaskDefinitionEntity(
                ...
                taskTitle = if (realGame != null) "挑战 ${realGame.name}" else "挑战 ${taskSpec.gameName}",
        ...
        Log.d(TAG, "已播种章节 $chapterId 任务: ${definitions.size} 个 (真实游戏偏移 $gameOffset, 可用 ${games.size} 个)")
    }
    ...
    private fun buildFallbackConfig(): AdventureFullConfig {
        ...
        val chapters = listOf(
            com.example.gameboxone.data.model.AdventureChapterSpec(
